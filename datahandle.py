...
if len(merged) > 0:
    rmse = np.sqrt(mean_squared_error(merged['Measured'], merged['Simulated']))
    r2 = r2_score(merged['Measured'], merged['Simulated'])
    print(f"="*30)
    ...
    if has_chinese_font:
        ...
        title_text = f"PV Fitting: Measured vs NASA Simulation\n(Location: Lufeng, R2={r2:.3f})"
        xlabel_text = "Date"
        ylabel_text = "Daily Energy (kWh)"
    plt.plot(merged.index, merged['Measured'], label=label_meas, color='blue', alpha=0.7)
    plt.plot(merged.index, merged['Simulated'], label=label_sim, color='red', linestyle='--', alpha=0.8)
    ...
    # 保存图片
    save_name = "Lufeng_PV_Fitting_Result.png"
